...
#define mnl_attr_for_each_nested(attr, nest)                             \
  for ((attr) = mnl_attr_get_payload(nest); mnl_attr_ok(                 \
           (attr), (char *)mnl_attr_get_payload(nest) +                  \
...
static void *mnl_nlmsg_get_payload_offset(const struct nlmsghdr *nlh,
                                          size_t offset) {
...
static bool mnl_nlmsg_portid_ok(const struct nlmsghdr *nlh,
                                unsigned int portid) {
...
static int mnl_attr_validate(const struct nlattr *attr,
...
static void mnl_attr_put_u16(struct nlmsghdr *nlh, uint16_t type,
                             uint16_t data) {
  mnl_attr_put(nlh, type, sizeof(uint16_t), &data);
}

static void mnl_attr_put_u32(struct nlmsghdr *nlh, uint16_t type,
                             uint32_t data) {
  mnl_attr_put(nlh, type, sizeof(uint32_t), &data);
}

static void mnl_attr_put_strz(struct nlmsghdr *nlh, uint16_t type,
                              const char *data) {
...
static struct nlattr *mnl_attr_nest_start_check(struct nlmsghdr *nlh,
...
static int __mnl_cb_run(const void *buf, size_t numbytes, unsigned int seq,
...
static int mnl_cb_run2(const void *buf, size_t numbytes, unsigned int seq,
                       ...
                       const mnl_cb_t *cb_ctl_array,
                       unsigned int cb_ctl_array_len) {
  return __mnl_cb_run(buf, numbytes, seq, portid, cb_data, data, cb_ctl_array,
                      cb_ctl_array_len);
}
...
static int mnl_socket_bind(struct mnl_socket *nl, unsigned int groups,
                           pid_t pid) {
  int ret;
  socklen_t addr_len;
...
static int mnlg_socket_send(struct mnlg_socket *nlg,
...
static int mnlg_socket_recv_run(struct mnlg_socket *nlg, mnl_cb_t data_cb,
  ...
  do {
    ...
    if (err <= 0) break;
    err = mnl_cb_run2(nlg->buf, err, nlg->seq, nlg->portid, data_cb, data,
...
static int get_family_id_attr_cb(const struct nlattr *attr, void *data) {
  ...
  if (mnl_attr_type_valid(attr, CTRL_ATTR_MAX) < 0) return MNL_CB_ERROR;
...
static int get_family_id_cb(const struct nlmsghdr *nlh, void *data) {
  ...
  if (!tb[CTRL_ATTR_FAMILY_ID]) return MNL_CB_ERROR;
...
static struct mnlg_socket *mnlg_socket_open(const char *family_name,
  ...
  nlh = __mnlg_msg_prepare(nlg, CTRL_CMD_GETFAMILY, NLM_F_REQUEST | NLM_F_ACK,
                           GENL_ID_CTRL, 1);
...
static int read_devices_cb(const struct nlmsghdr *nlh, void *data) {
  ...
  if (ret != MNL_CB_OK) return ret;
  ret = add_next_to_inflatable_buffer(buffer);
  if (ret < 0) return ret;
  if (nlh->nlmsg_type != NLMSG_DONE) return MNL_CB_OK + 1;
  return MNL_CB_OK;
}
...
another:
  ...
  if (len == MNL_CB_OK + 1) goto another;
  ret = 0;
...
static int add_del_iface(const char *ifname, bool add) {
  ...
  if (mnl_cb_run(rtnl_buffer, len, nlh->nlmsg_seq, mnl_socket_get_portid(nl),
                 NULL, NULL) < 0) {
    ret = -errno;
    goto cleanup;
  }
...
again:
  ...
  if (!peer) {
    ...
    if (dev->flags & WGDEVICE_HAS_PRIVATE_KEY)
      mnl_attr_put(nlh, WGDEVICE_A_PRIVATE_KEY, sizeof(dev->private_key),
                   dev->private_key);
    if (dev->flags & WGDEVICE_HAS_LISTEN_PORT)
      mnl_attr_put_u16(nlh, WGDEVICE_A_LISTEN_PORT, dev->listen_port);
    if (dev->flags & WGDEVICE_HAS_FWMARK)
      mnl_attr_put_u32(nlh, WGDEVICE_A_FWMARK, dev->fwmark);
    if (dev->flags & WGDEVICE_REPLACE_PEERS) flags |= WGDEVICE_F_REPLACE_PEERS;
    if (flags) mnl_attr_put_u32(nlh, WGDEVICE_A_FLAGS, flags);
  }
  if (!dev->first_peer) goto send;
  peers_nest = peer_nest = allowedips_nest = allowedip_nest = NULL;
  peers_nest = mnl_attr_nest_start(nlh, WGDEVICE_A_PEERS);
  for (i = 0, peer = peer ? peer : dev->first_peer; peer;
       peer = peer->next_peer) {
    uint32_t flags = 0;

    peer_nest = mnl_attr_nest_start_check(nlh, MNL_SOCKET_BUFFER_SIZE, i++);
    if (!peer_nest) goto toobig_peers;
    if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE, WGPEER_A_PUBLIC_KEY,
                            sizeof(peer->public_key), peer->public_key))
      goto toobig_peers;
    ...
    if (!allowedip) {
      ...
      if (peer->flags & WGPEER_HAS_PRESHARED_KEY) {
        if (!mnl_attr_put_check(
                nlh, MNL_SOCKET_BUFFER_SIZE, WGPEER_A_PRESHARED_KEY,
                sizeof(peer->preshared_key), peer->preshared_key))
          goto toobig_peers;
      }
      if (peer->endpoint.addr.sa_family == AF_INET) {
        if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE, WGPEER_A_ENDPOINT,
      ...
      if (peer->flags & WGPEER_HAS_PERSISTENT_KEEPALIVE_INTERVAL) {
        if (!mnl_attr_put_u16_check(nlh, MNL_SOCKET_BUFFER_SIZE,
                                    WGPEER_A_PERSISTENT_KEEPALIVE_INTERVAL,
    ...
    if (flags) {
      if (!mnl_attr_put_u32_check(nlh, MNL_SOCKET_BUFFER_SIZE, WGPEER_A_FLAGS,
                                  flags))
        goto toobig_peers;
    }
    if (peer->first_allowedip) {
      if (!allowedip) allowedip = peer->first_allowedip;
      allowedips_nest = mnl_attr_nest_start_check(nlh, MNL_SOCKET_BUFFER_SIZE,
                                                  WGPEER_A_ALLOWEDIPS);
      if (!allowedips_nest) goto toobig_allowedips;
      for (j = 0; allowedip; allowedip = allowedip->next_allowedip) {
        allowedip_nest =
            mnl_attr_nest_start_check(nlh, MNL_SOCKET_BUFFER_SIZE, j++);
        if (!allowedip_nest) goto toobig_allowedips;
        if (!mnl_attr_put_u16_check(nlh, MNL_SOCKET_BUFFER_SIZE,
                                    WGALLOWEDIP_A_FAMILY, allowedip->family))
          goto toobig_allowedips;
        if (allowedip->family == AF_INET) {
          if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE,
                                  WGALLOWEDIP_A_IPADDR, sizeof(allowedip->ip4),
                                  &allowedip->ip4))
            goto toobig_allowedips;
        } else if (allowedip->family == AF_INET6) {
          if (!mnl_attr_put_check(nlh, MNL_SOCKET_BUFFER_SIZE,
                                  WGALLOWEDIP_A_IPADDR, sizeof(allowedip->ip6),
                                  &allowedip->ip6))
            goto toobig_allowedips;
        }
        if (!mnl_attr_put_u8_check(nlh, MNL_SOCKET_BUFFER_SIZE,
                                   WGALLOWEDIP_A_CIDR_MASK, allowedip->cidr))
          goto toobig_allowedips;
  ...
  goto send;
toobig_allowedips:
  if (allowedip_nest) mnl_attr_nest_cancel(nlh, allowedip_nest);
  if (allowedips_nest) mnl_attr_nest_end(nlh, allowedips_nest);
  mnl_attr_nest_end(nlh, peer_nest);
  mnl_attr_nest_end(nlh, peers_nest);
  goto send;
toobig_peers:
  if (peer_nest) mnl_attr_nest_cancel(nlh, peer_nest);
  mnl_attr_nest_end(nlh, peers_nest);
  goto send;
send:
  if (mnlg_socket_send(nlg, nlh) < 0) {
...
static int parse_allowedips(const struct nlattr *attr, void *data) {
  ...
  if (!new_allowedip) return MNL_CB_ERROR;
  if (!peer->first_allowedip)
...
static int parse_peer(const struct nlattr *attr, void *data) {
  ...
  switch (mnl_attr_get_type(attr)) {
    ...
    case WGPEER_A_ENDPOINT: {
      ...
      if (mnl_attr_get_payload_len(attr) < sizeof(*addr)) break;
...
static int parse_peers(const struct nlattr *attr, void *data) {
  ...
  if (!new_peer) return MNL_CB_ERROR;
...
try_again:
  *device = calloc(1, sizeof(wg_device));
  ...
  nlh = mnlg_msg_prepare(nlg, WG_CMD_GET_DEVICE,
...
out:
  if (nlg) mnlg_socket_close(nlg);
  if (ret) {
    wg_free_device(*device);
    if (ret == -EINTR) goto try_again;
    *device = NULL;
  }
  errno = -ret;
  return ret;
...
int wg_key_from_base64(wg_key key, const wg_key_b64_string base64) {
  ...
  val = decode_base64((const char[]){base64[i * 4 + 0], base64[i * 4 + 1],
                                     base64[i * 4 + 2], 'A'});
  if (val < 0 || val & 0xff) goto out;
  key[i * 3 + 0] = (val >> 16) & 0xff;
  key[i * 3 + 1] = (val >> 8) & 0xff;
  errno = 0;
out:
